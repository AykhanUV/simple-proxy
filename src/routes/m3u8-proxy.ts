        ...
        if (line.startsWith("#")) {
          if (line.startsWith("#EXT-X-KEY:")) {
            ...
            if (uriMatch && uriMatch) {
              const keyUrl = parseURL(uriMatch, url);
              if (keyUrl) {
                const proxyKeyUrl = `${baseProxyUrl}/ts-proxy?url=${encodeURIComponent(
                  keyUrl,
                )}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
                newLines.push(line.replace(uriMatch, proxyKeyUrl));
              } else {
                newLines.push(line);
              }
            } else {
            ...
            const uriMatch = line.match(/URI="([^"]+)"/);
            if (uriMatch && uriMatch) {
              const mediaUrl = parseURL(uriMatch, url);
              if (mediaUrl) {
                const proxyMediaUrl = `${baseProxyUrl}/m3u8-proxy?url=${encodeURIComponent(
                  mediaUrl,
                )}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
                newLines.push(line.replace(uriMatch, proxyMediaUrl));
              } else {
                newLines.push(line);
              }
            } else {
